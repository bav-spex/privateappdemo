...
export const submitBatch = createAsyncThunk('appBatches/submitBatch', async params => {
    ...
    storePayload.facilitators = storePayload.facilitators.map(item => ({userId:item.userId,name:item.fullName, phone: item.mobileNumber,emailId:item.primaryEmail,status:"nominated"}))
    storePayload.teachers = storePayload.teachers.map(item => ({userId:item.userId,name:item.fullName, phone: item.mobileNumber,emailId:item.primaryEmail,status:"nominated"}))
    storePayload.observers = storePayload.observers.map(item => ({userId:item.userId,name:item.fullName, phone: item.mobileNumber,emailId:item.primaryEmail,status:"nominated"}))
...
export const batchSlice = createSlice({
    ...
    extraReducers: builder => {
      ...
      builder.addCase(getBatchList.fulfilled, (state, action) => {
        state.batchList = action.payload.items,
